  ...
  async listDocs(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      for (const sd of userRecord.secondaryDepartments) {
        if (sd.departmentId && !allDeptIds.includes(sd.departmentId)) {
          allDeptIds.push(sd.departmentId);
        }
      }
      ...
      const deptIdToCode: Record<string, string> = {};
      for (const d of departments) {
        deptIdToCode[d.id] = d.code;
      }

      const deptCodeToPermCode: Record<string, string> = {
      ...
      if (primaryDeptCode && deptCodeToPermCode[primaryDeptCode]) {
        userPermCodes.push(deptCodeToPermCode[primaryDeptCode]);
      }
      for (const sd of userRecord.secondaryDepartments) {
        const code = deptIdToCode[sd.departmentId];
        const mapped = code ? deptCodeToPermCode[code] : null;
        if (mapped && !userPermCodes.includes(mapped)) {
          userPermCodes.push(mapped);
        }
      }
  ...
  async getDocContent(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!fs.existsSync(filePath)) {
        res.status(404).json({ success: false, message: 'Không tìm thấy file tài liệu' });
        return;
      }
      ...
      res.json({
        success: true,
        data: { slug: doc.slug, title: doc.title, content },
      });
    } catch (error) {
      next(error);
    }
  ...
  async updateDocContent(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!userRecord || userRecord.role !== 'ADMIN') {
        res.status(403).json({ success: false, message: 'Chỉ admin mới có quyền chỉnh sửa tài liệu' });
        return;
      }
      ...
      if (typeof content !== 'string') {
        res.status(400).json({ success: false, message: 'Nội dung không hợp lệ' });
        return;
      }
