...
class TimesheetController {
  ...
  async upsertCell(req: Request, res: Response, next: NextFunction) {
    try {
      const { employeeId, date, code, note, workHours, overtimeHours } = req.body;
      const result = await timesheetService.upsertCell({ employeeId, date, code, note, workHours, overtimeHours });
