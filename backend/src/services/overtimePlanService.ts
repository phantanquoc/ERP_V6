...
import { OvertimePlan } from '@prisma/client';
import notificationService from './notificationService';

class OvertimePlanService {
  /** Map 1 user object sang DTO gọn */
  ...
  /** Populate 1 plan (dùng khi đã có userMap sẵn hoặc gọi đơn lẻ) */
  private async populateWithUsers(plan: OvertimePlan): Promise<any> {
    try {
      ...
      return { ...plan, nguoiTao: null, nguoiThamGia: [] };
    }
  }

  /** Populate nhiều plan với 1 lần query DB duy nhất */
  private async batchPopulateWithUsers(plans: OvertimePlan[]): Promise<any[]> {
    if (plans.length === 0) return [];
    try {
      const allIds = Array.from(new Set(plans.flatMap(p => [p.nguoiTaoId, ...p.nguoiThamGiaIds])));
      const users = await prisma.user.findMany({
        where: { id: { in: allIds } },
        select: { id: true, firstName: true, lastName: true, departmentId: true, employees: { select: { employeeCode: true } } },
      });
      ...
      return plans.map(p => ({ ...p, nguoiTao: null, nguoiThamGia: [] }));
    }
  }

  private buildPopulated(plan: OvertimePlan, userMap: Map<string, any>): any {
    const nguoiTao = userMap.get(plan.nguoiTaoId);
    return {
      ...plan,
      nguoiTao: nguoiTao ? this.mapUserDto(nguoiTao) : null,
      nguoiThamGia: plan.nguoiThamGiaIds
  ...
  async create(data: CreateOvertimePlanRequest, nguoiTaoId: string, files?: string[]): Promise<OvertimePlan> {
    const nguoiTao = await prisma.user.findUnique({
      where: { id: nguoiTaoId },
      include: { employees: true },
    });
    if (!nguoiTao) {
      throw new NotFoundError('Người tạo kế hoạch không tồn tại');
    }

    // Validate nguoiThamGia (employee IDs)
    const employees = await prisma.employee.findMany({
      where: { id: { in: data.nguoiThamGia } },
      select: { id: true, userId: true },
    });

    if (employees.length !== data.nguoiThamGia.length) {
    ...
    if (ngayTangCa < today) {
      throw new ValidationError('Ngày tăng ca phải từ ngày hôm nay trở đi');
    }
    ...
    // #4: Khi tạo chỉ gửi thông báo đến Admin để duyệt, KHÔNG gửi cho nhân viên
    // Nhân viên sẽ nhận thông báo SAU KHI admin duyệt (trong approvePlan)
    ...
    try {
      ...
      logger.error('Error sending overtime plan admin notifications:', error);
    }
  ...
  async getAll(query: OvertimePlanListQuery): Promise<{ plans: any[]; total: number; page: number; totalPages: number }> {
    ...
    if (query.nguoiTao) {
      where.nguoiTaoId = query.nguoiTao;
    }
    if (query.nguoiThamGia) {
      where.nguoiThamGiaIds = { has: query.nguoiThamGia };
    }
    if (query.department) {
      ...
      where.OR = [
        { nguoiTaoId: { in: userIds } },
        { nguoiThamGiaIds: { hasSome: userIds } },
      ];
    }
    ...
    const populated = await this.batchPopulateWithUsers(plans);

    return { plans: populated, total, page, totalPages: Math.ceil(total / limit) };
  }
  ...
  async update(id: string, data: UpdateOvertimePlanRequest, userId: string, files?: string[]): Promise<OvertimePlan> {
    const plan = await prisma.overtimePlan.findUnique({ where: { id } });
    if (!plan) throw new NotFoundError('Không tìm thấy kế hoạch tăng ca');
    if (plan.nguoiTaoId !== userId) {
      throw new ApiError(403, 'Chỉ người tạo mới có quyền cập nhật');
    }
    ...
    if (data.nguoiThamGia) {
      ...
      updateData.nguoiThamGiaIds = employees.map(emp => emp.userId);
    }
    ...
    if (files && files.length > 0) {
      updateData.files = [...plan.files, ...files];
    }
  ...
  async delete(id: string, userId: string): Promise<void> {
    ...
    if (!plan) throw new NotFoundError('Không tìm thấy kế hoạch tăng ca');
    if (plan.nguoiTaoId !== userId) {
      throw new ApiError(403, 'Chỉ người tạo mới có quyền xóa');
    }
  ...
  async acceptPlan(planId: string, userId: string, data: AcceptOvertimePlanRequest): Promise<any> {
    ...
    if (!plan) throw new NotFoundError('Không tìm thấy kế hoạch tăng ca');
    if (!plan.nguoiThamGiaIds.includes(userId)) {
      throw new ApiError(403, 'Bạn không phải người tham gia kế hoạch này');
    }

    const currentStatus = (plan.trangThaiTiepNhan as Record<string, string>) || {};
    currentStatus[userId] = data.trangThai;

    const updated = await prisma.overtimePlan.update({
      where: { id: planId },
  ...
  async approvePlan(planId: string, adminUserId: string, data: ApproveOvertimePlanRequest): Promise<any> {
    ...
    if (!adminUser || adminUser.role !== 'ADMIN') {
      throw new ApiError(403, 'Chỉ admin mới có quyền phê duyệt kế hoạch tăng ca');
    }

    const plan = await prisma.overtimePlan.findUnique({ where: { id: planId } });
    if (!plan) throw new NotFoundError('Không tìm thấy kế hoạch tăng ca');

    if (plan.trangThai !== 'CHO_DUYET') {
      throw new ValidationError('Kế hoạch tăng ca này đã được xử lý');
    }
    ...
    // #4: Gửi thông báo kết quả duyệt cho người tạo
    try {
      const adminName = `${adminUser.firstName} ${adminUser.lastName}`;
      const isApproved = newStatus === 'DA_DUYET';
      await notificationService.createNotification({
        userId: plan.nguoiTaoId,
        type: NotificationType.OVERTIME_PLAN,
        title: isApproved ? 'Kế hoạch tăng ca đã được duyệt' : 'Kế hoạch tăng ca bị từ chối',
        message: isApproved
          ? `${adminName} đã phê duyệt kế hoạch tăng ca: ${plan.noiDung}`
          : `${adminName} đã từ chối kế hoạch tăng ca: ${plan.noiDung}${data.lyDoTuChoi ? `. Lý do: ${data.lyDoTuChoi}` : ''}`,
      });

      // #4: Sau khi duyệt, gửi thông báo đến tất cả nhân viên tham gia
      if (isApproved) {
        for (const userId of plan.nguoiThamGiaIds) {
          if (userId !== plan.nguoiTaoId) { // Không gửi trùng cho người tạo
            await notificationService.createNotification({
              userId,
              type: NotificationType.OVERTIME_PLAN,
              title: 'Kế hoạch tăng ca đã được duyệt',
              message: `Kế hoạch tăng ca "${plan.noiDung}" đã được ${adminName} phê duyệt. Vui lòng kiểm tra lịch tăng ca.`,
            });
          }
        }
      }
    } catch (error) {
      logger.error('Error sending overtime plan approval notification:', error);
    }
