...
| 13 | Fix All Notification Workflows | ✅ | Audit toàn bộ 10 workflow — 9/10 đã có notification: nghỉ phép (tạo → QC, duyệt/từ chối → nhân viên), tăng ca (tạo → approver, duyệt/từ chối → người tạo + participants), giao việc (assign → người nhận, **mới**: accept/reject task → notify người giao, update task assignees → notify assignees mới), đơn hàng (status change → nhân viên phụ trách), bảng lương (publish → tất cả nhân viên), QC (phân công → QC personnel), phiếu nhập kho (status → requester), điều chỉnh vật tư (approve/reject → người yêu cầu), cuộc họp (create/update/cancel → participants + reminder). Check-in/checkout reminder covered by Task #14. | — |
| 14 | Attendance CardView + Reminder | ✅ | Backend: `/api/attendance/daily-summary` (employees grouped: `present_out`, `present`, `absent`) + cron 08:00/17:00/22:00 (WS push) đã có. Frontend: tab **Tổng quan ngày** trong `AttendanceManagement` với stat cards (tổng/hoàn thành/đang trong ca/vắng mặt) + 3 cột employee cards có avatar/tên/giờ, auto-refresh mỗi phút, real-time refresh qua WS event `ATTENDANCE_SUMMARY_CHANGED`. Notification khi admin thêm điểm danh LATE → notify nhân viên + supervisor bộ phận. | — |
