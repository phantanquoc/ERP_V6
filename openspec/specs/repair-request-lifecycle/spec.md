# repair-request-lifecycle Specification

## Purpose

Defines the RepairRequest lifecycle: typed forward-only status enum, dedicated business-event endpoints (start-repair, cancel, auto-complete via acceptance handover coverage), audit log, notification events, and frontend mutations and UI patterns for the repair workflow.
## Requirements
