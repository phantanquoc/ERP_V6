# output-entry-attribution Specification

## Purpose

Per-grade entry history for output weights, replacing the previous single-operator-per-record model. Records which worker entered each quality grade for each fry batch and machine, with soft employee references and waste-distribution exemption.
