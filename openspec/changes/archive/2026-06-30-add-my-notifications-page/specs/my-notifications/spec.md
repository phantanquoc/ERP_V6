...
- **AND** the response includes `total` (count of notifications in range), `unread` (count where `isRead=false` in range), `today` (count where `createdAt >= start-of-today server local in `Asia/Ho_Chi_Minh`), and `byType` (record of notification type → non-zero count within the same range)
...
The system SHALL centralize notification-type → route mapping in a single helper `resolveDeepLink(notification)` consumed by the detail modal. The mapping MUST cover:
