...
The system SHALL enforce a fixed allowlist of purchase-request status transitions: `Chờ báo giá → Chờ duyệt` (only via `POST /:id/submit-approval`), `Chờ duyệt → {Đã duyệt, Từ chối}`, `Đã duyệt → Hoàn thành` (gated: every item MUST have `giaThucTe >0` and `soLuongThucTe >0`; if any `soLuongThucTe != soLuong` then header `lyDoChenhLech` MUST be non-empty). Any `PUT /:id` that attempts a different `trangThai` transition or violates the Hoàn thành gate SHALL be rejected with `400 ValidationError` before any write.
...
#### Scenario: Hoàn thành without confirmed quantity or reason is rejected
- **WHEN** a `Đã duyệt` PR with any item missing `soLuongThucTe` or `giaThucTe`, or with `soLuongThucTe != soLuong` but empty `lyDoChenhLech`, attempts `PUT ... {trangThai: "Hoàn thành"}`
- **THEN** the server responds `400` with a message indicating the missing confirmed field or reason
