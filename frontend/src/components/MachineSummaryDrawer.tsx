import { useEffect } from 'react';
import { X } from 'lucide-react';
import Portal from './Portal';
import { useMachineSummary } from '../hooks/useMachines';

interface MachineSummaryDrawerProps {
  machineId: string | null;
  onClose: () => void;
}

const statusBadge = (status: string) => {
  const map: Record<string, { label: string; cls: string }> = {
    HOAT_DONG: { label: 'Hoạt động', cls: 'bg-green-100 text-green-700' },
    'BẢO_TRÌ': { label: 'Bảo trì', cls: 'bg-yellow-100 text-yellow-700' },
    'NGỪNG_HOẠT_ĐỘNG': { label: 'Ngừng HĐ', cls: 'bg-red-100 text-red-700' },
  };
  const cfg = map[status] || map.HOAT_DONG;
  return <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${cfg.cls}`}>{cfg.label}</span>;
};

const formatDate = (value?: string | null) =>
  value ? new Date(value).toLocaleDateString('vi-VN') : '—';

const MachineSummaryDrawer = ({ machineId, onClose }: MachineSummaryDrawerProps) => {
  const { data: summary, isLoading } = useMachineSummary(machineId ?? '');
  const isOpen = !!machineId;

  useEffect(() => {
    if (!isOpen) return;
    const handleEsc = (e: KeyboardEvent) => { if (e.key === 'Escape') onClose(); };
    document.addEventListener('keydown', handleEsc);
    return () => document.removeEventListener('keydown', handleEsc);
  }, [isOpen, onClose]);

  if (!isOpen) return null;

  return (
    <Portal>
      <div className="fixed inset-0 z-[9999]" onClick={onClose}>
        <div className="absolute inset-0 bg-black/30" />
        <div
          className="absolute right-0 top-0 h-full w-full max-w-lg bg-white shadow-xl"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
              <h2 className="text-lg font-semibold text-gray-900">Tổng quan máy</h2>
              <button onClick={onClose} className="rounded p-1.5 text-gray-500 hover:bg-gray-100">
                <X className="h-5 w-5" />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              {isLoading ? (
                <div className="text-center text-gray-500 py-8">Đang tải...</div>
              ) : !summary ? (
                <div className="text-center text-gray-500 py-8">Không tìm thấy dữ liệu</div>
              ) : (
                <>
                  <section>
                    <h3 className="text-sm font-semibold text-gray-700 mb-2">Thông tin máy</h3>
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                      <dt className="text-gray-500">Mã máy</dt>
                      <dd className="font-medium text-blue-700">{summary.maMay}</dd>
                      <dt className="text-gray-500">Tên máy</dt>
                      <dd className="font-medium text-gray-900">{summary.tenMay}</dd>
                      <dt className="text-gray-500">Trạng thái</dt>
                      <dd>{statusBadge(summary.trangThai)}</dd>
                      <dt className="text-gray-500">Mô tả</dt>
                      <dd className="text-gray-700">{summary.moTa || '—'}</dd>
                      <dt className="text-gray-500">Ghi chú</dt>
                      <dd className="text-gray-700">{summary.ghiChu || '—'}</dd>
                    </dl>
                  </section>

                  <section>
                    <h3 className="text-sm font-semibold text-gray-700 mb-2">
                      Lỗi gần nhất ({summary.faultRecords?.length ?? 0})
                    </h3>
                    {!summary.faultRecords?.length ? (
                      <p className="text-xs text-gray-400">Không có</p>
                    ) : (
                      <ul className="space-y-1.5">
                        {summary.faultRecords.map((f: any) => (
                          <li key={f.id} className="rounded border border-gray-100 bg-gray-50 px-3 py-2 text-xs">
                            <span className="font-medium text-gray-800">{f.maLoi}</span>
                            <span className="ml-2 text-gray-600">{f.tenLoi}</span>
                            <span className="ml-2 text-gray-400">{formatDate(f.ngayPhatHien)}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                  </section>

                  <section>
                    <h3 className="text-sm font-semibold text-gray-700 mb-2">
                      Yêu cầu sửa chữa ({summary.repairRequestItems?.length ?? 0})
                    </h3>
                    {!summary.repairRequestItems?.length ? (
                      <p className="text-xs text-gray-400">Không có</p>
                    ) : (
                      <ul className="space-y-1.5">
                        {summary.repairRequestItems.map((r: any) => (
                          <li key={r.id} className="rounded border border-gray-100 bg-gray-50 px-3 py-2 text-xs">
                            <span className="font-medium text-gray-800">{r.repairRequest?.maYeuCau ?? '—'}</span>
                            <span className="ml-2 text-gray-600">{r.noiDungLoi}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                  </section>

                  <section>
                    <h3 className="text-sm font-semibold text-gray-700 mb-2">
                      Vận hành gần nhất ({summary.systemOperations?.length ?? 0})
                    </h3>
                    {!summary.systemOperations?.length ? (
                      <p className="text-xs text-gray-400">Không có</p>
                    ) : (
                      <ul className="space-y-1.5">
                        {summary.systemOperations.map((op: any) => (
                          <li key={op.id} className="rounded border border-gray-100 bg-gray-50 px-3 py-2 text-xs">
                            <span className="font-medium text-gray-800">{op.loaiVanHanh}</span>
                            <span className="ml-2 text-gray-600">{op.noiDung}</span>
                            <span className="ml-2 text-gray-400">{formatDate(op.ngayVanHanh)}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                  </section>
                </>
              )}
            </div>
          </div>
        </div>
      </div>
    </Portal>
  );
};

export default MachineSummaryDrawer;