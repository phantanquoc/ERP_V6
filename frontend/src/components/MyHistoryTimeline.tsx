...
interface MyHistoryTimelineProps {
  ...
  totalPages: number;
  isLoading: boolean;
  onItemClick: (item: HistoryItem, buttonEl?: HTMLButtonElement) => void;
  onPageChange: (page: number) => void;
...
const MyHistoryTimeline: React.FC<MyHistoryTimelineProps> = ({
  ...
  // Per-day expansion state — Set<dayKey>; resets when items reference changes
  ...
  if (total === 0) {
      ...
      return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p className="text-gray-700 text-sm font-semibold mb-1">Không có hoạt động nào khớp bộ lọc</p>
            <p className="text-gray-400 text-xs mb-4">Thử mở rộng thời gian hoặc xóa bộ lọc</p>
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="flex flex-col items-center justify-center py-16 text-center px-4">
          <Inbox className="w-12 h-12 text-gray-300 mb-3" />
          <p className="text-gray-700 text-sm font-semibold mb-1">Chưa có hoạt động nào trong 30 ngày qua</p>
          <p className="text-gray-400 text-xs mb-4">Bạn chưa có hoạt động nào được ghi nhận</p>
          <button
            type="button"
            onClick={onExpandYear}
            className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {/* Timeline body with vertical rail line */}
