...
const ProcessManagement: React.FC<ProcessManagementProps> = ({ mode = 'full' }) => {
  ...
  const filteredProcesses = useMemo(() => processes.filter(process => {
    ...
    if (f.maQuyTrinh && !(process.maQuyTrinh || '').toLowerCase().includes(String(f.maQuyTrinh).toLowerCase())) return false;
    if (f.tenQuyTrinh && !(process.tenQuyTrinh || '').toLowerCase().includes(String(f.tenQuyTrinh).toLowerCase())) return false;
    if (f.loaiQuyTrinh && process.loaiQuyTrinh !== f.loaiQuyTrinh) return false;
    return true;
  }), [processes, tableFilters]);

  const paginatedProcesses = useMemo(() => {
    const start = (tablePage - 1) * TABLE_PAGE_SIZE;
    return filteredProcesses.slice(start, start + TABLE_PAGE_SIZE);
  }, [filteredProcesses, tablePage]);
  ...
  const columns: Column<Process>[] = [
    {
      ...
      label: 'STT',
      render: (process) => {
        const idx = paginatedProcesses.indexOf(process);
        return (tablePage - 1) * TABLE_PAGE_SIZE + idx + 1;
      },
    },
    {
      key: 'maQuyTrinh',
      ...
      filterable: true,
      filterType: 'text',
      render: (process) => <span className="font-semibold text-blue-600">{process.maQuyTrinh}</span>,
    },
    {
      key: 'tenQuyTrinh',
      label: 'Tên quy trình',
      filterable: true,
      filterType: 'text',
      render: (process) => <span className="text-gray-700">{process.tenQuyTrinh}</span>,
    },
    {
      key: 'msnv',
      label: 'MSNV',
      render: (process) => process.msnv,
    },
    {
      key: 'tenNhanVien',
      label: 'Tên nhân viên',
      render: (process) => <span className="font-medium text-gray-900">{process.tenNhanVien}</span>,
    },
    {
      key: 'loaiQuyTrinh',
      ...
      filterOptions: [
        ...
        { label: 'Kiểm tra chất lượng', value: 'Kiểm tra chất lượng' },
        { label: 'Đóng gói', value: 'Đóng gói' },
        { label: 'Vận chuyển', value: 'Vận chuyển' },
        { label: 'Khác', value: 'Khác' },
      ],
      render: (process) => process.loaiQuyTrinh,
    },
    {
      key: 'actions',
      label: 'Thao tác',
    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <DataTable
          ...
          onFilterChange={(filters) => { setTableFilters(filters); setTablePage(1); }}
          rowKey="id"
