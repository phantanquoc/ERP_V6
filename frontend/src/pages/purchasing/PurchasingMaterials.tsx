 ...
 *
 * `replenishmentRequestId` is deep-linked by the YCBS queue below, and `orderId`
 * is declared so a hand-off into the order tab cleans up behind it; an
 * undeclared key is never touched.
 *
  ...
  useEffect(() => {
    const fetchPRStats = async () => {
      try {
        ...
        // Tổng chi năm: sum(giaDuKien * soLuong) qua items[]
        setYearSpend(
          yearList.reduce((s: number, pr: any) => {
            const items = (pr as any).items ?? [];
            return s + items.reduce((a: number, it: any) => a + (Number(it.soLuong) || 0) * (Number(it.giaDuKien) || 0), 0);
          }, 0)
        );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div
          ...
          className="bg-white rounded-xl shadow-sm border border-gray-100 hover:border-blue-200 hover:shadow-md transition-all cursor-pointer p-5"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Nhà cung cấp · NVL</h3>
        ...
        <div
          ...
          className="bg-white rounded-xl shadow-sm border border-gray-100 hover:border-emerald-200 hover:shadow-md transition-all cursor-pointer p-5"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">
                  Yêu cầu mua hàng · NVL · <span className="text-gray-900">{cardPRStats.total} trong năm {selectedYear}</span>
                </h3>
            ...
            {/* Trend vs tháng trước (so tháng selectedMonth) */}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h4 className="text-sm font-semibold text-gray-800">Số yêu cầu theo tháng — {selectedYear} · NVL</h4>
              ...
              >
                ...
                <Bar dataKey="count" radius={[6, 6, 0, 0]} maxBarSize={36} isAnimationActive={false}>
                  {Array.from({ length: 12 }, (_, i) => (
                    <Cell key={i} fill={i + 1 === selectedMonth ? '#2563eb' : '#93c5fd'} stroke={i + 1 === selectedMonth ? '#1e40af' : 'transparent'} strokeWidth={i + 1 === selectedMonth ? 1 : 0} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="mt-2 flex items-center justify-center gap-4 text-[11px] text-gray-500">
          <span className="inline-flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: '#2563eb' }} /> Tháng đã chọn</span>
          <span className="inline-flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: '#93c5fd' }} /> Tháng khác</span>
          <span className="text-slate-400">· Màu theo thực thể (tháng), không theo thứ hạng</span>
        </div>
        {/* Data table view for a11y — hidden visually but available */}</div>
                              ...
                              <tr key={it.id ?? idx} className="align-top">
                                ...
                                <td className="px-2 py-2">
                                  <input
                                    ...
                                    placeholder="0"
                                    className="w-full px-2 py-1 border border-gray-200 rounded text-sm text-right focus:outline-none focus:ring-1 focus:ring-green-500"
                                  />
                                </td>
