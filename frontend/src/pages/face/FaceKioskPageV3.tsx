...
type FacePos    = 'none' | 'detected' | 'multiface';

const MIN_FACE_AREA         = 0.03;
const POST_SCAN_COOLDOWN_MS  = 2500;
const BURST_FRAME_COUNT      = 3;
const BURST_FRAME_INTERVAL_MS = 120;
...
const FaceKioskPageV3: React.FC = () => {
  ...
  const captureBurst = useCallback(async (): Promise<string[]> => {
    const frames: string[] = [];
    for (let i = 0; i < BURST_FRAME_COUNT; i++) {
      const f = captureFrame();
      if (f) frames.push(f);
      if (i < BURST_FRAME_COUNT - 1) {
        await new Promise(r => setTimeout(r, BURST_FRAME_INTERVAL_MS));
      }
    }
    return frames;
  }, [captureFrame]);
  ...
  const doScan = useCallback(async () => {
    ...
    try {
      const res = kioskConfig.deviceKey
        ? await faceAttendanceService.kioskVerify(best, frames, kioskConfig.deviceKey, kioskConfig.deviceId, 'relaxed')
        : isLocalDev
          ? await faceAttendanceService.kioskVerifyDev(best, frames, 'relaxed')
          : (() => { throw new Error('Thiết bị chưa được cấu hình device key'); })();
      if (res.data) {
        if (res.data.topK?.length) {
          console.group('[face-kiosk-v3] TopK matches');
  ...
  }, [captureBurst, showResult, isLocalDev, kioskConfig.deviceId, kioskConfig.deviceKey]);
  ...
  const drawLoop = useCallback(async () => {
    ...
    if (detection) {
      ...
      const sinceLastScan = now - lastScanAt.current;
      const cooldownDone  = sinceLastScan >= POST_SCAN_COOLDOWN_MS;
      const bigEnough     = faceAreaRatio >= MIN_FACE_AREA;

      if (bigEnough && cooldownDone && !processing.current) {
        doScan();
      }

      const color = bigEnough ? '#22d3ee' : '#f59e0b';
    ...
    rafRef.current = window.setTimeout(drawLoop, detectInterval.current);
  }, [captureFrame, doScan, enterSleep]);
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
          <div className="flex flex-col items-center">
            ...
            <p className="mt-4 text-white/70 text-base font-medium tracking-wide drop-shadow">
              Đưa mặt vào camera để chấm công
            </p>
          </div>
        </div>
      )}
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10 flex flex-col justify-center items-center gap-3 pt-16"
          ...
          {facePos === 'detected' && (
            <div className="flex items-center gap-3 bg-black/50 backdrop-blur-sm px-5 py-2.5 rounded-full border border-cyan-400/60">
              ...
              <span className="text-sm font-medium text-cyan-200">Đang chấm công...</span>
            </div>
          )}
        </div>
      )}
...
export default FaceKioskPageV3;